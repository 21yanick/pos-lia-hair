...
export function useSales() {
  ...
  const createSale = async (data: CreateSaleData) => {
    console.log('🔥 POS useSales: createSale called with data:', data)
    try {
      setLoading(true)
      setError(null)

      // 1. Organization ID prüfen (Multi-Tenant Security)
      console.log('🔍 POS useSales: currentOrganization:', currentOrganization)
      ...
      // 3. Verkauf erstellen (MIT organization_id!)
      const saleData = {
      ...
      console.log('🚨 POS useSales: About to insert sale with data:', saleData)
