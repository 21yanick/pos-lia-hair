-- =====================================================
-- 08_fix_unified_transactions_view_v6.sql
-- =====================================================
-- Fix: unified_transactions_view should only show ACTIVE documents
-- (not archived PDFs with replacement_reason != NULL)
-- MINIMAL CHANGE: Only 2 lines modified in document joins
-- =====================================================
...
CREATE OR REPLACE VIEW public.unified_transactions_view AS
 WITH transaction_base AS (
         SELECT s.id,
            ...
            s.customer_name
           FROM sales s
             LEFT JOIN documents d ON d.reference_id = s.id AND d.type = 'receipt'::text AND d.replacement_reason IS NULL
        ...
        UNION ALL
         SELECT e.id,
           ...
           FROM expenses e
             LEFT JOIN documents d ON d.reference_id = e.id AND d.type = 'expense_receipt'::text AND d.replacement_reason IS NULL
        UNION ALL
         SELECT cm.id,
...
-- =====================================================
-- END OF 08_fix_unified_transactions_view_v6.sql
-- =====================================================
-- ✅ PRODUCTION-SAFE: CREATE OR REPLACE is atomic
-- ✅ MINIMAL CHANGE: Only 2 lines modified (document joins)
-- ✅ NO DATA LOSS: Views don't store data
-- ✅ BACKWARDS COMPATIBLE: Same columns as before
-- =====================================================
