...
interface UseExpensesQueryReturn {
  ...
  loadCurrentMonthExpenses: () => Promise<{
    success: boolean
    expenses?: ExpenseWithSupplier[]
    error?: string
  }>
...
export function useExpensesQuery(): UseExpensesQueryReturn {
  ...
  const stableQueryKey = useMemo(() => {
    return organizationId
      ? queryKeys.business.expenses.list(organizationId)
      : ['expenses', 'disabled']
  }, [organizationId])
  ...
  const updateExpenseMutation = useMutation({
    ...
    onMutate: async ({ updates }) => {
      ...
      // Optimistically update
      queryClient.setQueryData(queryKey, (old: ExpenseWithSupplier[] = []) =>
        old.map((expense) => (expense.id === updates.id ? { ...expense, ...updates } : expense))
      )
  ...
  const loadExpensesByDateRangeWrapper = async (startDate: string, endDate: string) => {
    try {
      ...
      const dateRangeQueryKey = queryKeys.business.expenses.dateRange(
        organizationId,
        startDate,
        endDate
      )
