...
export async function updateSupplier(id: string, updates: Partial<SupplierFormData>, organizationId: string): Promise<Supplier> {
  ...
  if (updates.contact_phone !== undefined) updateData.contact_phone = updates.contact_phone?.trim() || null
  if (updates.website !== undefined) updateData.website = updates.website?.trim() || null
  if (updates.address_line1 !== undefined) updateData.address_line1 = updates.address_line1?.trim() || null
  if (updates.address_line2 !== undefined) updateData.address_line2 = updates.address_line2?.trim() || null
  if (updates.postal_code !== undefined) updateData.postal_code = updates.postal_code?.trim() || null
...
export async function deleteSupplier(id: string, organizationId: string): Promise<void> {
  ...
  if (expenses && expenses.length > 0) {
    throw new Error('Lieferant kann nicht gelöscht werden, da er in Ausgaben verwendet wird. Deaktivieren Sie ihn stattdessen.')
  }
...
export async function getSupplierWithStats(id: string, organizationId: string): Promise<SupplierWithStats | null> {
  ...
  const last_expense_date = expenses.length > 0 
    ? expenses.reduce((latest: string, exp: any) => 
...
export function categorizeSupplier(supplierName: string): SupplierCategory {
  ...
  // Beauty & Supplies
  if (normalized.includes('newflag') || 
  ...
  // Real Estate
  if (normalized.includes('immobilien') ||
  ...
  // Insurance
  if (normalized.includes('versicherung') ||
