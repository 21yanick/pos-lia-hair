...
export async function importItems(
  ...
  const existingItems = await supabase.from('items').select('name')
...
export async function importUsers(
  ...
  const existingUsers = await supabase.from('users').select('email, username')

  const existingEmails = new Set(existingUsers.data?.map((user) => user.email.toLowerCase()) || [])
  const existingUsernames = new Set(
    existingUsers.data?.map((user) => user.username.toLowerCase()) || []
  )
...
export async function importBankAccounts(
  ...
  const { data, error } = await supabase.from('bank_accounts').insert(accountsWithUserId).select()
...
export async function importSuppliers(
  ...
  const existingSuppliers = await supabase.from('suppliers').select('name').eq('user_id', userId)

  const existingNames = new Set(
    existingSuppliers.data?.map((supplier) => supplier.name.toLowerCase()) || []
  )
  ...
  const { data, error } = await supabase.from('suppliers').insert(suppliersWithUserId).select()
...
export async function getItemIdByName(itemName: string): Promise<string | null> {
  const { data, error } = await supabase.from('items').select('id').eq('name', itemName).limit(1) // Take first match if duplicates exist
...
export async function generateCashMovements(
  ...
  if (movements.length > 0) {
    const { data, error } = await supabase.from('cash_movements').insert(movements).select()
...
export async function calculateDailySummariesForImport(
  ...
  for (const date of uniqueDates) {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        status: 'closed',
        cash_ending: 200.0, // Placeholder - should be calculated properly
