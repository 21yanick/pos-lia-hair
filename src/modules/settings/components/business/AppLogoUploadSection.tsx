...
import { Loader2, Upload, Trash2, AlertCircle, ImageIcon, CheckCircle, Sun, Moon } from 'lucide-react'
...
export function AppLogoUploadSection() {
  ...
  // Handle file upload
  const handleFileUpload = useCallback(async (file: File, theme: LogoTheme) => {
    setError(null)
  ...
  // Handle drop
  const handleDrop = useCallback((e: React.DragEvent, theme: LogoTheme) => {
    e.preventDefault()
    e.stopPropagation()
    setDragActive(null)
  ...
  // Handle delete logo
  const handleDeleteLogo = useCallback(async (theme: LogoTheme) => {
    setError(null)
    try {
      ...
      <div className="space-y-4">
        ...
        {hasLogo && (
          <div className="flex items-center space-x-4 p-4 border border-input rounded-lg bg-background">
            <div className={`w-16 h-16 flex items-center justify-center border border-input rounded-lg overflow-hidden ${
            ...
            <div className="flex-1">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">{theme === 'light' ? 'Helles' : 'Dunkles'} Logo hochgeladen</span>
              </div>
          ...
          <div className="space-y-4">
            {previewUrl ? (
              <div className="flex flex-col items-center space-y-2">
                <div className={`w-24 h-24 flex items-center justify-center border border-input rounded-lg overflow-hidden ${
                  theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'
                }`}>
                  <img 
                    src={previewUrl} 
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Laden Sie Logos für helle und dunkle Themes hoch. Diese werden in der Navigation, beim Login und in der App angezeigt.
        </CardDescription>
      </CardHeader>
