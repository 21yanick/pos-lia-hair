...
export function useAppointmentCalendar(initialDate: Date = new Date()): UseAppointmentCalendarReturn {
  // Core state
  ...
  // Dialog Management
  const openBookingDialog = useCallback((slot: TimeSlot) => {
    handleSlotClick(slot)
  }, [handleSlotClick])
  
  const openAppointmentDialog = useCallback((appointment: AppointmentBlock) => {
  ...
  const isSameMonth = useCallback((date: Date): boolean => {
