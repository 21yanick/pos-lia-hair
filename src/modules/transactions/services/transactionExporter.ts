...
export function exportTransactionsToCSVWithMetadata(
  ...
  // Transaction Headers
  const headers: (keyof TransactionCsvRow)[] = [
    'datum', 'zeit', 'typ', 'belegnummer', 'beschreibung', 
    'betrag', 'zahlungsart', 'status', 'pdf_status'
  ]
...
function getTypeDisplayName(type: string, typeCode: string): string {
  switch (type) {
    case 'sale': return `Verkauf (${typeCode})`
    case 'expense': return `Ausgabe (${typeCode})`
    case 'cash_movement': return `Kassenbewegung (${typeCode})`
    case 'bank_transaction': return `Bank-Transaktion (${typeCode})`
...
function getStatusDisplayName(status: string): string {
  switch (status) {
    ...
    case 'unmatched': return 'Unabgeglichen'
    case 'matched': return 'Abgeglichen'
...
function getPdfStatusDisplayName(pdfStatus: string): string {
  switch (pdfStatus) {
    ...
    case 'not_needed': return 'Nicht nötig'
    case 'generating': return 'Wird erstellt'
    default: return pdfStatus
  }
}
