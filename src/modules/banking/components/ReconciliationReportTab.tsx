...
import { FileText, Download, Calendar, AlertCircle, CheckCircle, Users, Building2, ChevronLeft, ChevronRight, History, ExternalLink } from "lucide-react"
...
export function ReconciliationReportTab() {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const salesCash = sales?.filter(s => s.payment_method === 'cash').reduce((sum, s) => sum + s.total_amount, 0) || 0
      const salesTwint = sales?.filter(s => s.payment_method === 'twint').reduce((sum, s) => sum + s.total_amount, 0) || 0
      const salesSumup = sales?.filter(s => s.payment_method === 'sumup').reduce((sum, s) => sum + s.total_amount, 0) || 0
      const salesTotal = salesCash + salesTwint + salesSumup
      
      // Berechne vollständige Ausgaben-Stats
      const expensesCash = expenses?.filter(e => e.payment_method === 'cash').reduce((sum, e) => sum + e.amount, 0) || 0
      const expensesBank = expenses?.filter(e => e.payment_method === 'bank').reduce((sum, e) => sum + e.amount, 0) || 0
      ...
      {showArchive ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Bereits erstellte Monatsberichte herunterladen
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {reconciliationData.bankReconciliation.summary.totalBankTransactions} abgeglichen
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 max-h-64 sm:max-h-80 lg:max-h-96 overflow-y-auto">
                ...
                {reconciliationData.providerReconciliation.matches.map((match, index) => (
                  <div key={index} className="flex items-center space-x-3 p-3 bg-green-50 dark:bg-green-950/20 rounded-lg border-l-4 border-green-500">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ↔ {match.provider.provider.toUpperCase()}: {match.provider.netAmount.toFixed(2)} CHF
                      </p>
                    </div>
                  </div>
                ))}
