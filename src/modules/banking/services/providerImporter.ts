...
export async function previewProviderFile(file: File): Promise<ProviderImportPreview> {
  try {
    ...
    if (detection.provider === 'unknown') {
      throw new Error('Could not detect provider type. Please ensure this is a valid TWINT or SumUp CSV file.')
    }
    ...
    const totalAmount = duplicateCheck.newRecords.reduce((sum, record) => sum + record.gross_amount, 0)
    ...
    if (duplicateCheck.duplicateRecords.length > 0) {
      warnings.push(`${duplicateCheck.duplicateRecords.length} duplicate transactions found and will be skipped`)
    }
    if (invalidRecords.length > 0) {
      warnings.push(`${invalidRecords.length} records have validation errors and will be skipped`)
    }
    if (detection.confidence < 0.8) {
      warnings.push(`Provider detection confidence is low (${Math.round(detection.confidence * 100)}%)`)
    }
    
    return {
      filename: file.name,
      provider: detection.provider,
      detectedFormat: detection.provider === 'twint' ? 'TWINT CSV Report' : 'SumUp Transactions Report',
    ...
    throw new Error(`Failed to preview provider file: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export async function importProviderFile(
  ...
  try {
    ...
    throw new Error(`Failed to import provider file: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
