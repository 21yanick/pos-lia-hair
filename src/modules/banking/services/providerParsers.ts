...
export function detectProviderType(csvContent: string): ProviderDetectionResult {
  ...
  if (content.includes('gutgeschriebener betrag'))
    twintIndicators.push('Header: Gutgeschriebener Betrag')
  if (content.includes('kennung verkaufsstelle'))
    twintIndicators.push('Header: Kennung Verkaufsstelle')
  ...
  if (lines.some((line) => line.includes('umsatz')))
    sumupIndicators.push('Content: Umsatz transactions')
